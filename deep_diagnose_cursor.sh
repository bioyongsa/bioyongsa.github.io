...
if [ -d "$CURSOR_SERVER_DIR" ]; then
    ...
    # 하위 디렉토리 확인
    echo "   하위 디렉토리:"
    ls -ld "$CURSOR_SERVER_DIR"/* 2>/dev/null | awk '{print "     -", $9, "("$1")"}'
else
    echo "   존재: ❌"
    echo "   생성 시도 중..."
...
echo ""
echo "💡 다음 단계:"
echo "   1. 로그 파일의 오류 메시지 확인"
echo "   2. Cursor IDE에서 'Remote-Containers: Show Container Log' 실행"
echo "   3. Windows 이벤트 로그 확인 (선택사항)"
